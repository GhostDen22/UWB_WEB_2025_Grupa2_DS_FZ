...
export const ShopProvider = ({ children }) => {
  ...
  const handlePlaceOrder = (onSuccess) => {
    ...
    fetch('http://localhost:5000/orders', {
      ...
      .then((data) => {
        ...
        const newOrderRecord = {
          ...
          title: `Zamówienie nr ${data.orderId}`,
          price: totalSum,
          items: [...cart]
        };

        setOrders(prevOrders => {
          const isDuplicate = prevOrders.some(order => order.id === newOrderRecord.id);
          if (isDuplicate) return prevOrders;
          return [...prevOrders, newOrderRecord];
        });
